...
interface TempLevel { label: string; hint: string; colour: string }
...
function getDefaultChatSession(
  ...
  return { conversationId: null, lastResponse: null, maxTokens: 1000, messages: buildStarterMessages(specialistName), specialistId, streamEnabled: false, temperature }
}
...
function readChatSession(): ChatSession {
  ...
  try {
    ...
    const p = JSON.parse(raw) as Partial<ChatSession>
    return {
      conversationId: typeof p.conversationId === 'string' ? p.conversationId : null,
      lastResponse: p.lastResponse ?? null,
      maxTokens: typeof p.maxTokens === 'number' ? p.maxTokens : 1000,
      messages: Array.isArray(p.messages) && p.messages.length > 0 ? p.messages : buildStarterMessages('General Assistant'),
      specialistId: typeof p.specialistId === 'string' ? p.specialistId : DEFAULT_SPECIALIST_ID,
      streamEnabled: typeof p.streamEnabled === 'boolean' ? p.streamEnabled : false,
      temperature: typeof p.temperature === 'number' ? p.temperature : DEFAULT_TEMPERATURE,
    }
  } catch { return getDefaultChatSession() }
}
...
export function ChatPage() {
  ...
  useEffect(() => {
    const raf = window.requestAnimationFrame(() => {
      ...
      setLastResponse(s.lastResponse)
      setSpecialistId(s.specialistId)
      setStreamEnabled(s.streamEnabled)
  ...
  useEffect(() => {
    const chip = chipRowRef.current?.querySelector(`[data-id="${specialistId}"]`)
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/specialists`, { signal: controller.signal })
      .then(res => { if (!res.ok) throw new Error(''); return readSpecialistsResponse(res) })
  ...
  }, [conversationId, isSessionReady, lastResponse, maxTokens, messages, specialistId, streamEnabled, temperature])
  ...
  async function sendMessage(event: SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(`${apiBaseUrl}/api/chat`, {
        ...
        body: JSON.stringify({ conversation_id: conversationId, message: trimmed, max_tokens: maxTokens, specialist: specialistId, temperature })
      })
      if (res.status === 429) throw new Error(`Rate limit reached. Please wait ${parseRetryAfter(res)}s before trying again.`)
  ...
  async function sendStreamingMessage(optimisticMessage: ChatMessage) {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/chat/stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversation_id: conversationId, message: optimisticMessage.content, max_tokens: maxTokens, specialist: specialistId, temperature })
      })
      if (res.status === 429) throw new Error(`Rate limit reached. Please wait ${parseRetryAfter(res)}s before trying again.`)
      if (!res.ok) throw new Error(await readErrorMessage(res))
      ...
      for await (const event of readChatStream(res)) {
        if (event.type === 'text') {
          setMessages(current => current.map(m => m === assistantMessage ? { ...m, content: `${m.content}${event.text}` } : m))
          continue
        }
      ...
      if (!finalResponse) throw new Error('Claude ended the stream before sending a final response.')
    } catch (err) {
  ...
  function resetChat(nextSpecialistId?: string, nextTemperature?: number) {
    try { window.sessionStorage.removeItem(chatSessionKey) } catch { }
    const id = nextSpecialistId ?? specialistId
    const name = specialists.find(s => s.id === id)?.name ?? 'General Assistant'
  ...
  function handleSpecialistChange(nextId: string) {
    ...
    const preset = specialists.find(s => s.id === nextId)?.temperature ?? DEFAULT_TEMPERATURE
  ...
  return (
    <main className="relative flex min-h-screen flex-col">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col gap-5 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
        ...
        <nav className="animate-fade-in flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Button
              ...
              className="border-white/15 bg-white/4 transition-all hover:-translate-y-0.5 hover:border-orange-200/30"
              onClick={() => { resetChat() }}
            >
        ...
        {/* ── Header ──────────────────────────────────────────────────────── */}
        <header className="animate-fade-in grid gap-5 lg:grid-cols-[1fr_auto]">

          {/* Title + description */}
          <div>
            <h1 className="text-3xl/tight font-semibold text-foreground sm:text-4xl">
              Chat with Claude
            </h1>
            <p className="mt-1.5 max-w-xl text-sm/6 text-muted-foreground">
              Pick a specialist below, tune the response style, then start a conversation.
              Context is stored on the server — follow-ups just work.
            </p>
          </div>

          {/* Response style slider (compact, lives in top-right corner) */}
          <div className="flex w-full flex-col gap-1.5 rounded-xl border border-white/10 bg-black/20 p-3 backdrop-blur-sm lg:w-56">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className={`text-xs font-semibold ${tl.colour}`}>{tl.label}</span>
            </div>
            <input
              id={temperatureInputId}
              type="range" min="0" max="1" step="0.05" value={temperature}
              className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-orange-400"
              onChange={e => { setTemperature(Number(e.target.value)) }}
            />
            <div className="flex justify-between text-[10px] text-muted-foreground/40">
              <span>Factual</span><span>Creative</span>
            </div>
            <p className="text-[11px] italic text-muted-foreground/50">{tl.hint}</p>
          </div>
        </header>
        ...
        <div className="animate-slide-up-fade flex flex-col gap-2">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isActive
                      ? 'border-orange-300/40 bg-orange-300/12 text-orange-100 shadow-lg shadow-orange-950/20 -translate-y-0.5'
                      : 'border-white/10 bg-white/4 text-muted-foreground hover:border-white/20 hover:bg-white/7 hover:text-foreground hover:-translate-y-0.5'
                    }
                  `}
                  aria-pressed={isActive}
                >
                  <span className="text-base leading-none" aria-hidden="true">{ui?.emoji ?? '🤖'}</span>
          ...
          {currentSpecialist ?
            <p className="pl-1 text-xs text-muted-foreground/60">
              <span className="font-medium text-muted-foreground/80">{currentSpecialist.name}</span>
              {' '}—{' '}
        ...
        {error ?
          <div
            className="animate-slide-up-fade rounded-lg border border-rose-500/25 bg-rose-500/8 px-4 py-3 text-sm text-rose-200"
            role="alert"
          >
            {error}
          </div>
        ...
        {/* ── Chat window ──────────────────────────────────────────────────── */}
        <div className="flex min-h-[56vh] flex-1 flex-col rounded-2xl border border-white/10 bg-white/[0.03] shadow-2xl shadow-black/25 backdrop-blur-md">

          {/* Thread header */}
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/8 bg-black/15 px-4 py-3">
            <div className="flex items-center gap-3">
              <div className="grid size-9 shrink-0 place-items-center rounded-xl border border-orange-200/15 bg-orange-300/10 text-lg">
            ...
            {lastResponse ?
              <p className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs text-muted-foreground/70">
          ...
          <div
            ...
            style={{ maskImage: 'linear-gradient(to bottom, transparent, black 18px, black calc(100% - 18px), transparent)' }}
            role="log"
            aria-live="polite"
          ...
          >
            {messages.map((message, index) => (
              <div
                ...
                style={{ animation: 'message-in 0.35s cubic-bezier(0.22, 1, 0.36, 1) both', animationDelay: `${Math.min(index * 35, 240)}ms` }}
              >
                {/* Assistant avatar */}
                {message.role === 'assistant' ?
                  <div className="mt-1 grid size-8 shrink-0 place-items-center rounded-full border border-white/10 bg-black/25 text-sm">
                    {currentUiConfig?.emoji ?? '🤖'}
                  </div>
                  : null}
                ...
                <article
                  className={`
                    group max-w-[88%] rounded-2xl border px-4 py-3 transition-shadow duration-300
                    hover:shadow-xl sm:max-w-[75%]
                    ${message.role === 'user'
                      ? 'border-orange-200/20 bg-orange-300/10 text-orange-50 shadow-orange-950/10'
                ...
                {/* User avatar */}
                {message.role === 'user' ?
                  <div className="mt-1 grid size-8 shrink-0 place-items-center rounded-full border border-orange-200/15 bg-orange-300/10 text-orange-100">
            ...
            {isSending && !streamEnabled ?
              <div
                className="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-muted-foreground"
                role="status"
            ...
            {hasOnlyStarter ?
              <div className="animate-slide-up-fade grid gap-2 pt-2 sm:grid-cols-3">
                {starterPrompts.map(prompt => (
                  <button
                    ...
                    className="
                      min-h-16 rounded-xl border border-white/10 bg-black/12 px-3.5 py-3
                      text-left text-xs/5 text-muted-foreground/70 transition-all
                      hover:-translate-y-0.5 hover:border-orange-200/25 hover:bg-orange-300/8
          ...
          {/* ── Input form ─────────────────────────────────────────────────── */}
          <form onSubmit={sendMessage} className="grid gap-3 border-t border-white/8 bg-black/12 p-4">
            <label htmlFor={messageInputId} className="sr-only">Message</label>
            <p id={messageHelpId} className="sr-only">Press Enter to send. Press Shift and Enter to add a line break.</p>
            <Textarea
              id={messageInputId}
              ref={textareaRef}
              value={draft}
              ...
              className="
                max-h-52 min-h-24 resize-none rounded-xl border-white/15 bg-black/18
                p-4 text-sm/6 transition-all placeholder:text-muted-foreground/50
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <label className="flex items-center gap-2 text-xs text-muted-foreground focus-within:text-foreground" htmlFor={tokenInputId}>
                  Max tokens
                  <input
                    id={tokenInputId}
                    type="number" min={50} max={4000} value={maxTokens}
                    className="h-8 w-24 rounded-md border border-white/10 bg-black/20 px-2 text-sm text-foreground transition-colors hover:border-white/20 focus:border-orange-200/35 focus:outline-none"
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground/60">
                  <div className="h-1.5 w-16  rounded-full bg-white/10">
                    <div
                      className="h-full rounded-full bg-orange-300/70 transition-all duration-300"
                      style={{ width: `${draftUsage}%` }}
                    />
                  </div>
                  <span className="tabular-nums">{draft.length}/{MAX_DRAFT_LENGTH}</span>
                </div>

                {/* Stream toggle */}
                <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-white/10 bg-black/15 px-2.5 py-1.5 text-xs text-muted-foreground transition-colors hover:border-orange-200/20 hover:text-foreground">
                  <input
                    type="checkbox"
              ...
              {/* Right controls */}
              <div className="flex items-center gap-3">
                <p className="hidden items-center gap-1.5 text-xs text-muted-foreground/45 sm:flex">
                ...
                <Button
                  ...
                  disabled={!canSend}
                  className="
                    min-w-24 bg-orange-500 text-white shadow-lg shadow-orange-950/30
