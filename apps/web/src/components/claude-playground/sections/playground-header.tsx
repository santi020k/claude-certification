...
export function PlaygroundHeader({
  ...
  return (
    <header className="animate-fade-in flex flex-col gap-5">
      ...
      <nav className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            disabled={isChecking}
            className="border-white/10 bg-white/5 transition-all duration-200 hover:border-white/20 hover:bg-white/10"
          >
            {isChecking ?
          ...
          {health ?
            (
              <div className="animate-scale-in flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.035] px-3 py-2 text-xs text-muted-foreground">
      ...
      {/* ── Page title ───────────────────────────────────────────────────── */}
      <div>
        <h1 className="text-3xl/tight font-semibold text-foreground">Prompt Playground</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {EXAMPLE_PROMPTS.slice(0, 2).map(prompt => (
          <button
            ...
            className="
              group flex items-center gap-1.5 rounded-full border border-white/10
              bg-white/[0.035] px-4 py-1.5 text-xs text-muted-foreground
              transition-all duration-200
              hover:border-orange-500/25 hover:bg-orange-500/8 hover:text-orange-200
            "
          >
