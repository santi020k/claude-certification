...
const markdownComponents: Components = {
  ...
  h4: ({ children }) => (
    <h4
      className="
        mt-5 text-sm font-semibold tracking-wide text-muted-foreground uppercase
        first:mt-0
      "
    >
      {children}
  ...
  code: ({ children, className }) => {
    ...
    if (isBlock) {
      return (
        <code
          className="
            block rounded-xl border border-white/8 bg-[#15110e] p-4 font-mono
            text-xs/6 text-orange-50/90
          "
        >
          {children}
        </code>
    ...
    return (
      <code
        className="
          rounded-md border border-white/8 bg-white/4 px-1.5 py-0.5 font-mono
          text-[0.85em] text-orange-100
        "
      >
        {children}
      </code>
  ...
  a: ({ children, href }) => (
    <a
      ...
      className="
        text-orange-200 underline decoration-orange-300/40 underline-offset-4
        transition
